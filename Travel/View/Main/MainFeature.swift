    ...
    //            state.isSheetCancelButtonTapped = false
            ...
            case .setRoutes(let routes):
                state.routes = routes
                state.isSheetPresented = true
                // ✅ Always auto-select the fastest route after rerouting
                   if let fastest = routes.min(by: { $0.expectedTravelTime < $1.expectedTravelTime }) {
                       state.selectedRoute = fastest
                       state.routeSteps = fastest.steps
                   }
                return .none
            ...
            case .setGoButtonTapped(let route):
                ...
                state.selectedRoute = route
                state.routeSteps = state.selectedRoute?.steps ?? []
